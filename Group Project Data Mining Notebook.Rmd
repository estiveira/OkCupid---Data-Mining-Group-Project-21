...
```{r - Model Evaluation for the imputaion with mice}
# Model evaluation 
...
```{r - Model Evaluation for the imputaion with mean}
# Model evaluation 

predictions_mean = as.data.frame(class::knn(train = train_norm_mean[, c(1, 3:7, 9:13)], test = train_norm_mean[, c(1, 3:7, 9:13)], cl = train_norm_mean[, 15], k = 3))
confusion_matrix_mean = table(as.numeric(as.character(predictions_mean[, 1])), train_norm_mean[, 15])
...
mean(recall_mean)
f1_score_mean = 2 * (precision_mean * recall_mean) / (precision_mean + recall_mean)
mean(f1_score_mean)
...
# Using the model created with the data imputed with mice, because the values for precision, recall and f1-score are higher
